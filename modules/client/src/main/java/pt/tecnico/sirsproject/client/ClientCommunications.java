...
public final class ClientCommunications {
    ...
    public static String connect_to_sensors(String request, String requestType, String handler, String address,
                                            String port, TrustManager[] trustManagers) {
        System.out.println("Connecting to Sensors on address " + address + " and port: " + port + "...");
        
        String result = "";
        try {
            result = SendRequest.sendRequest(address, port, request, requestType, handler, trustManagers);
        } catch (NoSuchAlgorithmException | KeyManagementException | IOException e) {
            System.out.println("Error: Failed to connect to the Sensors.");
            System.exit(1);
        }
        return result;
    }
    
    public static String connect_to_frontoffice(String request, String requestType, String handler, String address,
                                               String port, TrustManager[] trustManagers) {
        System.out.println("Connecting to FrontOffice on address " + address + " and port: " + port + "...");
        String result = "";
        try {
            result = SendRequest.sendRequest(address, port, request, requestType, handler, trustManagers);
        } catch (NoSuchAlgorithmException | KeyManagementException | IOException e) {
            System.out.println("Error: Failed to connect to the FrontOffice.");
            System.exit(1);
        }
        return result;
    }
